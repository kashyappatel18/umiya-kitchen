...
class pinvoice extends CI_Controller{
	...
	function index(){
		if($this->session->userdata('logged_in')){
			...
			$data['contacts']=$this->Contact_Model->getContactByType("Supplier");
			...
			$this->load->view('pages/pinvoice_view',$data);
		...
		if($this->form_validation->run()==FALSE){
			...
			$this->Tax_Model->insert($data['tax']);//insert above data to cld_tax_meta
                        ...
                        $support=array(
                            'vno'=>$vno,
                            'sno'=>1,
                            'dname'=>'Invoice No:'.$this->input->post('inv_no'),
                            'f_year'=>$this->session->userdata['logged_in']['f_year'],
                            'v_id'=>$v_id,
                            'user_id'=>$this->session->userdata['logged_in']['id']
                        );
                        ...
                        $details=array(
                                    ...
                                    'naration'=>'Invoice No:'.$this->input->post('inv_no').' from '.$name,
                                    'v_id'=>$v_id,
                                    'f_year'=>$this->session->userdata['logged_in']['f_year'],
                                    'user_id'=>$this->session->userdata['logged_in']['id']
                            );
                        ...
                        $details=array(
                                    ...
                                    'ac_code'=>227001,
                                    'amount'=>($gtot-$tot),
                                    'naration'=>'Tax on Purchase Invoice No:'.$data['inv_data']['inv_no'].' from '.$name,
                                    'f_year'=>$this->session->userdata['logged_in']['f_year'],
                                    'v_id'=>$v_id,
                                    'user_id'=>$this->session->userdata['logged_in']['id']
                                );
